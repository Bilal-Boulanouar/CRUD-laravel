...
<body>
<div class="container">
<h1>Mise a jour l'Étudiant {{ $etudiant->id }}</h1>
<form action="{{ route('etudiants.update', $etudiant->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        @if($etudiant->image && file_exists(storage_path('app/public/' . $etudiant->image)))
            <div>
                <img src="{{ asset('storage/' . $etudiant->image) }}" alt="Image de l'étudiant" class="img-thumbnail" style="max-width: 150px;">
            </div>
        @endif
    </div>
    ...
    <div class="form-group">
        ...
        <input type="date" name="date_naissance" id="date_naissance" class="form-control @error('date_naissance') is-invalid @enderror" value="{{ old('date_naissance', $etudiant->date_naissance) }}">
    ...
    <div class="form-group">
        ...
        <select name="genre" id="genre" class="form-control @error('genre') is-invalid @enderror">
            <option value="Homme" {{ old('genre', $etudiant->genre) == 'Homme' ? 'selected' : '' }}>Homme</option>
            <option value="Femme" {{ old('genre', $etudiant->genre) == 'Femme' ? 'selected' : '' }}>Femme</option>
        </select>
        @error('genre')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    ...
    <div class="form-group">
        ...
        <input type="text" name="niveau_etude" id="niveau_etude" class="form-control @error('niveau_etude') is-invalid @enderror" value="{{ old('niveau_etude', $etudiant->niveau_etude) }}">
        @error('niveau_etude')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
